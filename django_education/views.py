...
from .models import Utilisateur, sequence, sequence_info, famille_competence, competence, cours, cours_info,\
    ...
    application, note_suivi, AutoEvaluation
...
def rentree_scolaire():
    rentree = datetime.now(tz=timezone.utc)
    unjour = timedelta(days=1)
    while rentree.month!=8:
        rentree -= unjour
    rentree += unjour
    return rentree
    ...
    if request.method == 'POST':
        if 'upload_eleves' in request.POST:
            ...
            for ligne1 in lignes[1:-1]:
                ...
                etudiant = Etudiant(user=user, annee='PTSI', lv1=langue_vivante.objects.get(langue='Anglais'),promotion=current_dateTime.strftime("%Y"))
                etudiant.save()
...
def lister_ressources_info(request):
    ...
    dossier_ds = github + 'Informatique/tree/main/DS/'
...
def afficher_sequence_si(request, id_sequence):
    ...
    videoyoutubes=videoyoutube.objects.filter(sequence__numero=id_sequence)
    applications=application.objects.filter(sequence__numero=id_sequence)
    ...
    return render(request, 'sequence.html', {'sequence':sequence_a_afficher,'courss':courss,'tds':tds,'tps':liste_ilots,'kholes':kholes,'quizzes':quizzes,'applications':applications,\
    'videos':videos,'videoyoutubes':videoyoutubes,'si':True})
...
def afficher_sequence_info(request):
    courss=cours_info.objects.all()
    tds=td_info.objects.all()
    tps=tp_info.objects.all()
    quizzes=Quiz.objects.filter(category__category="Info")
    return render(request, 'informatique.html', {'courss':courss,'tds':tds,'tps':tps,'quizzes':quizzes,'info':True})
...
def autoevaluationlistseq(request, id_etudiant, id_sequence):
    ressources=ressource.objects.filter(sequence=id_sequence)
    sequences=sequence.objects.all()
    competences = competence.objects.filter(Q(ressource__in=ressources)).distinct()
    liste_competences=[]
    for comp in competences:
        comp2 = comp.__dict__
        autoeval=AutoEvaluation.objects.filter(etudiant=id_etudiant,competence=comp)
        if autoeval.exists():
            comp2['note']=autoeval[0].note
        else:
            comp2['note']=''
        liste_competences.append(comp2)
    if request.user.is_teacher or (request.user.is_student and request.user.id==id_etudiant):
        context = {
           'chart': DetailsCharts(id_etudiant),'etudiant':id_etudiant,'sequence_selected':id_sequence,'sequences':sequences,'competences': liste_competences,'autoevaluation': True,
        }
        return render(request, 'resultats.html', context)
    else:
        return render(request, '/index')
...
@csrf_exempt
def autoevaluationsave(request,id_sequence,id_etudiant):
    data = {'success': False}
    if request.method=='POST':
        comp=request.POST['comp'][4:]
        notetosave=request.POST['note'].replace(',','.')
        etudianttosave=Etudiant.objects.get(user=id_etudiant)
        competencetosave=competence.objects.get(id=comp)
        if notetosave=='':
            AutoEvaluation.objects.filter(etudiant=etudianttosave,competence=competencetosave).delete()
            data['success'] = True
            data['value'] = ''
        else:
            if float(notetosave)>5:
                notetosave='5.0'
            try:
                AutoEvaluation.objects.update_or_create(etudiant=etudianttosave,competence=competencetosave ,
                            defaults={'note': float(notetosave)})
                data['success'] = True
                data['value'] = notetosave
            except:
                data['success'] = False
                data['value'] = ''
...
#     i,j=0,0
#     while j < len(cours):
#         date=date_rentree+timedelta(days=7*i)
#         if date < date.today():
#             color="#cccdcd"
#         elif date <= date.today() and date>date.today()-timedelta(days=7):
#             color="#03a5f6"
#         else:
#             color=""
#         if d.is_holiday_for_zone(date, 'C') and d.is_holiday_for_zone(date+timedelta(days=1), 'C') and date.month!=8:
...
#             j+=1
#         i+=1
#     return render(request, 'progression_2.html', {'semaines':semaines})
