...
class systeme(models.Model):
    ...
    def url(self):
        return remove_accents(github + 'Sciences-Ingenieur/tree/main/Systemes/' + self.nom + '/')
...
class fichier_systeme(models.Model):
    ...
    def url_fichier(self):
        return remove_accents(github_raw + 'Sciences-Ingenieur/master/Systemes/' + self.systeme.nom + '/' + \
...
def url(self,matiere,lien,type,ilot):
    if matiere=='si':
        if ilot==0:
            dossier_raw = github_raw + 'Sciences-Ingenieur/master/' + str("S%02d" % self.sequence.numero) + ' ' + \
              self.sequence.nom + '/' + type + ("%02d" % self.numero) \
              + " " + self.nom + "/"
            dossier = github + 'Sciences-Ingenieur/tree/main/' + str("S%02d" % self.sequence.numero) + ' ' + \
              self.sequence.nom + '/' + type + ("%02d" % self.numero) \
              + " " + self.nom + "/"
            if lien == 'git':
               return dossier
            elif lien == 'pdf':
                return dossier_raw + str("%02d" % self.sequence.numero) + '-' + type + ("%02d" % self.numero) + ".pdf"
            elif lien == 'prive':
                return dossier_raw + str("%02d" % self.sequence.numero) + '-' + type + ("%02d" % self.numero) + "_prive.pdf"
        else:
            dossier_raw = github_raw + 'Sciences-Ingenieur/master/' + str("S%02d" % self.tp.sequence.numero) + ' ' + \
              self.tp.sequence.nom + '/' + type + ("%02d" % self.tp.numero) \
              + " " + self.tp.nom + "/Ilot_" + ("%02d" % self.numero) + " " + self.systeme.nom + "/"
            dossier = github + 'Sciences-Ingenieur/tree/main/' + str("S%02d" % self.tp.sequence.numero) + ' ' + \
              self.tp.sequence.nom + '/' + type + ("%02d" % self.tp.numero) \
              + " " + self.tp.nom + "/Ilot_" + ("%02d" % self.numero) + " " + self.systeme.nom + "/"
            if lien == 'git':
               return dossier
            elif lien == 'pdf':
                return dossier_raw + str("%02d" % self.tp.sequence.numero) + '-' + type + ("%02d" % self.tp.numero) + '-I' + ("%02d" % self.numero) + ".pdf"
            elif lien == 'prive':
                return dossier_raw + str("%02d" % self.tp.sequence.numero) + '-' + type + ("%02d" % self.tp.numero) + '-I' + ("%02d" % self.numero) + "_prive.pdf"
    else:
        if type == 'C':
            nom_type = 'Cours'
        else:
            nom_type = type
        dossier_raw = github_raw + 'Informatique/master/' + nom_type + '/' + type + ("%02d" % self.numero) \
          + " " + self.nom + "/"
        dossier = github + 'Informatique/tree/master/' + nom_type + '/' + type + ("%02d" % self.numero) \
          + " " + self.nom + "/"
        if lien == 'git':
            return dossier
        elif lien == 'pdf':
            return dossier_raw + 'I-' + type + ("%02d" % self.numero) + ".pdf"
        elif lien == 'prive':
            return dossier_raw + 'I-' + type + ("%02d" % self.numero) + "_prive.pdf"
        elif lien == 'python':
...
class video(models.Model):
    ...
    def url(self):
        dossier = github_raw + 'Sciences-Ingenieur/master/' + str("S%02d" % self.ressource.sequence.numero) + ' ' + \
...
class videoyoutube(models.Model):
    ...
    def __str__(self):
        return str("S%02d" % self.sequence.numero)+'-'+str("%02d" % self.numero)+' '+self.nom
...
class AutoEvaluation(models.Model):
    ...
    note=models.FloatField(blank=True,validators = [MinValueValidator(0.0),MaxValueValidator(5.0)])

    def __str__(self):
        return self.etudiant.user.last_name+' '+self.etudiant.user.first_name+' '+str(self.competence.reference)
...
def url_ds(self, lien):
    dossier_raw = github_raw + 'Sciences-Ingenieur/master/DS/' + annee_scolaire(self.date) +'/'+ self.type_de_ds + ("%02d" % self.numero) + "/"
    dossier = github + 'Sciences-Ingenieur/tree/main/DS/' + annee_scolaire(self.date) +'/'+ self.type_de_ds + ("%02d" % self.numero) + "/"
    if lien == 'git':
       return dossier
    elif lien == 'pdf':
        return dossier_raw + self.type_de_ds + str("%02d" % self.numero) + ".pdf"
    elif lien == 'prive':
        return dossier_raw + self.type_de_ds + str("%02d" % self.numero) + "_prive.pdf"
...
class tp_data(models.Model):
    ilot=models.ForeignKey('ilot', on_delete=models.CASCADE)
    data=models.JSONField()

    def __str__(self):
        return self.ilot.nom_tp()
